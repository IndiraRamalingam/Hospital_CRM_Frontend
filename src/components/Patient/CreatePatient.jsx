...
function createPatient() {
  ...
  const[specialistt,setSpecialistt]=useState([])
  ...
  const getDoctorDetails = async() =>{
    try{
        ...
        console.log(response.data.alldoctors[0])
         setSpecialistt(response.data.alldoctors)
         setSpecialist(response.data.alldoctors[0])
  ...
  const handleCreate =(event) =>
  {
    ...
    {
      createDoctor({name,password,email,specialist,age,address,phone}) 
